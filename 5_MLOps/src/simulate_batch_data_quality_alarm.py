"""Simulate a Batch Data Quality alarm with drifted batch input."""
...
from .start_custom_batch_data_quality_job import start_custom_batch_data_quality_job
...
def simulate_batch_data_quality_alarm(
    *,
    wait: bool = False,
    poll_seconds: int = 30,
    ...
    drift_uri = _ensure_drift_s3_uri(config)
    job = start_custom_batch_data_quality_job(
        wait=wait,
        poll_seconds=poll_seconds,
        timeout_seconds=timeout_seconds,
        current_data_s3_uri=drift_uri,
    )
    payload = {
        "scenario": "batch_data_quality_alarm",
        "intent": "Drifted batch input records are compared against the Data Quality baseline to force violations.",
        "drift_batch_input_s3_uri": drift_uri,
        "custom_batch_data_quality_job": job,
        "expected_alarm": config.custom_batch_data_quality_alarm_name,
        "expected_metric": {
        ...
        "next_steps": [
            "Ensure python -m src.create_batch_cloudwatch_alarm was executed.",
    ...
    write_metadata(config, "batch_data_quality_alarm_simulation", payload)
    return payload
...
def main() -> None:
    ...
    print(
        json.dumps(
            simulate_batch_data_quality_alarm(
                wait=args.wait,
