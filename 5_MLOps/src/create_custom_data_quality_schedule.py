"""Create an EventBridge cron fallback that starts custom Data Quality jobs."""
...
def _validate_eventbridge_schedule_expression(expression: str) -> None:
    if expression.strip().upper() == "NOW":
        raise ValueError(
            "CUSTOM_DATA_QUALITY_CRON_EXPRESSION must be a cron expression for EventBridge. "
            "Use `python -m src.start_custom_data_quality_job --wait` for a manual run."
        )
...
def _lambda_environment(config, *, image_uri: str, instance_type: str, code_s3_uri: str) -> dict[str, str]:
    environment = {
        **custom_data_quality_environment(config),
        ...
        "JOB_PREFIX": f"{config.resource_prefix}-custom-data-quality",
...
def _upsert_lambda(clients, config, *, image_uri: str, instance_type: str, code_s3_uri: str) -> str:
    if not config.lambda_execution_role_arn:
        raise RuntimeError("LAMBDA_EXECUTION_ROLE_ARN is required to create the custom Data Quality trigger.")
    source = Path("lambdas/custom_data_quality_trigger.py")
    zip_path = _zip_lambda(source, config.local_outputs_dir / f"{config.custom_data_quality_trigger_lambda_name}.zip")
    ...
    try:
        response = clients.lambda_client.create_function(
            FunctionName=config.custom_data_quality_trigger_lambda_name,
            Runtime="python3.12",
        ...
        _wait_for_lambda_ready(clients, config.custom_data_quality_trigger_lambda_name)
        ...
        _lambda_write_with_retries(
            ...
            function_name=config.custom_data_quality_trigger_lambda_name,
        ...
        description = _wait_for_lambda_ready(clients, config.custom_data_quality_trigger_lambda_name)
...
def _allow_eventbridge_to_invoke_lambda(clients, config, *, function_arn: str, rule_arn: str) -> None:
    statement_id = f"{config.custom_data_quality_schedule_name}-invoke"
    try:
...
def create_custom_data_quality_schedule(
    *,
    ...
    if if_native_unavailable and not force:
        native_metadata = read_metadata(config, "monitoring_schedule")
        native_status = str(native_metadata.get("status") or "")
        if native_status != "native_schedule_unavailable":
            payload = {
                "skipped": True,
                "reason": "native_data_quality_schedule_available",
                "native_status": native_status or "unknown",
            }
            write_metadata(config, "custom_data_quality_schedule", payload)
            return payload

    _validate_eventbridge_schedule_expression(config.custom_data_quality_cron_expression)
    ...
    response = clients.events.put_rule(
        Name=config.custom_data_quality_schedule_name,
        ScheduleExpression=config.custom_data_quality_cron_expression,
        State="ENABLED",
        Description="Fallback cron that starts a custom Data Quality Processing Job.",
    ...
    clients.events.put_targets(
        Rule=config.custom_data_quality_schedule_name,
        Targets=[{"Id": "CustomDataQualityTrigger", "Arn": function_arn}],
    )
    payload = {
        "status": "created",
        "schedule_name": config.custom_data_quality_schedule_name,
        "schedule_expression": config.custom_data_quality_cron_expression,
        "rule_arn": rule_arn,
        "trigger_lambda_name": config.custom_data_quality_trigger_lambda_name,
...
def main() -> None:
    ...
    print(
        json.dumps(
            create_custom_data_quality_schedule(
