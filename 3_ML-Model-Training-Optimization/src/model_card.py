...
def main() -> None:
    ...
    lines = [
        ...
        f"- Raw source: {state.get('raw_data_s3_uri', config.raw_data_s3_uri)}",
        f"- Cleaned source: {state.get('cleaned_data_s3_uri', config.cleaned_data_s3_uri)}",
        f"- Curated feature source: {state.get('curated_features_s3_uri', config.curated_features_s3_uri)}",
        "- Record identifier: customer_id",
        "- Event time feature: event_time",
        "- Target column: churn_label",
        "- Training source: Feature Store Offline Store materialized through Athena, with curated S3 snapshot as fallback.",
