...
def main() -> None:
    ...
    lines = [
        ...
        f"- Raw data: {state.get('raw_data_s3_uri', config.raw_data_s3_uri)}",
        f"- Cleaned data: {state.get('cleaned_data_s3_uri', config.cleaned_data_s3_uri)}",
        f"- Curated features: {state.get('curated_features_s3_uri', config.curated_features_s3_uri)}",
        f"- Offline Store: {config.offline_store_s3_uri}",
        f"- Feature ingestion Processing Job: {state.get('feature_ingestion_job_name')}",
